...
import { MyDateTimePicker, TextRow, ConfirmationDialog } from "../../shared/components";
...
export function MatchDetails({ route, navigation }) {
  ...
  const onDeleteAction = () => {
    ...
    setCofirmSubmitAction(() => async () => {
      const result = await matchesService.deleteMatch(
        match?.id
      );
      setShowConfirmDialog(false);
      if (result.success === false) {
        ...
        const matches = await matchesService.getMatches({tournamentId: tournament.id});
        setMatches(matches);
    ...
    <View className="flex-1 pt-3">
      ...
      <View className="flex-1 px-3">
        ...
        <View className="my-3 flex-row justify-between">
          <TouchableOpacity onPress={onDeleteAction} className="flex-row p-2 bg-red-500 rounded-lg items-center">
            <Text className="text-white">Видалити</Text>
            <FeatherIcon color="white" name="trash-2" size={16} />
          </TouchableOpacity>
          <TouchableOpacity onPress={onEditAction} className="flex-row p-2 bg-gray-200 rounded-lg items-center">
